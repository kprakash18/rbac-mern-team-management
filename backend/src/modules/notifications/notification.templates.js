export const NotificationTemplates = {
  TASK_ASSIGNED: ({ taskTitle, actorName }) => ({
    title: "Task Assigned",
    message: actorName ? `Task "${taskTitle}" was assigned to you by ${actorName}.` : `Task "${taskTitle}" was assigned to you.`,
  }),
  TASK_STATUS_CHANGED: ({ taskTitle, status, actorName }) => ({
    title: "Task Status Changed",
    message: actorName ? `Task "${taskTitle}" status was changed to ${status} by ${actorName}.` : `Task "${taskTitle}" status was changed to ${status}.`,
  }),
  USER_ROLE_CHANGED: ({ roleName, details }) => ({
    title: "Role Changed",
    message: roleName ? `Your role has been changed to ${roleName}.` : (details || "Your role assignment has been updated."),
  }),
  USER_STATUS_CHANGED: ({ status, teamName, details }) => ({
    title: "Account Status Changed",
    message: status && teamName ? `Your membership in ${teamName} is now ${status.toLowerCase()}.` : (details || "Your account or membership status has been updated."),
  }),
  USER_ACCESS_CHANGED: ({ permissionName, resource, details }) => ({
    title: "Access Permissions Changed",
    message: permissionName ? `Your access for '${permissionName}' on ${resource || "resource"} has changed.` : (details || "Your permissions or access grants have been modified."),
  }),
  ...
  ROLE_REVOKED: ({ roleName }) => ({
    title: "Role Revoked",
    message: roleName ? `Your '${roleName}' role has been revoked.` : "One of your assigned roles has been revoked.",
  }),
  ...
  TASK_REMARKS_UPDATED: ({ taskTitle, actorName, preview }) => ({
    ...
    message: actorName ? `${actorName} updated the progress note on task '${taskTitle}'${preview ? `: "${preview}"` : "."}` : `The progress note on task '${taskTitle}' was updated${preview ? `: "${preview}"` : "."}`,
  }),
  CHANNEL_ADDED: ({ channelName, actorName }) => ({
    title: "Added to Channel",
    message: actorName ? `You were added to channel #${channelName} by ${actorName}.` : `You were added to channel #${channelName}.`,
  }),
  INVITATION_RECEIVED: ({ teamName, inviterName }) => ({
    title: "New Team Invitation",
    message: inviterName ? `${inviterName} invited you to join team ${teamName}.` : `You have been invited to join team ${teamName}.`,
  }),
  ...
  ACCESS_GRANTED: ({ permissionKey }) => ({
    ...
    message: permissionKey ? `Your access request for '${permissionKey}' has been approved.` : "Your access request has been approved.",
  }),
  ...
  ACCESS_REVOKED: ({ permissionKey }) => ({
    ...
    message: permissionKey ? `Your access grant for '${permissionKey}' has been revoked.` : "Your temporary access grant has been revoked.",
  }),

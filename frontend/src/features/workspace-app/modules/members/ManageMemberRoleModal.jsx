...
export default function ManageMemberRoleModal({ isOpen, member, teamId, onClose, onSaveRole }) {
  ...
  const [roles, setRoles] = useState(() => workspaceRoles || []);
  const [loadingRoles, setLoadingRoles] = useState(() => !workspaceRoles || workspaceRoles.length === 0);
  ...
  // Use bootstrap workspace roles or fetch fallback
  useEffect(() => {
    if (!isOpen) return;
    ...
    if (workspaceRoles && workspaceRoles.length > 0) {
      ...
      const matched = workspaceRoles.find(
        (r) =>
          r._id === member?.roleId ||
      ...
      if (matched) {
        ...
        setSelectedRoleId(workspaceRoles[0]._id);
      }
      return;
    }
  ...
  }, [isOpen, member, effectiveTeamId, workspaceRoles]);
      ...
      <div className="w-full max-w-xl bg-surface-container-lowest rounded-xl shadow-2xl overflow-hidden border border-border-subtle animate-in zoom-in-95 duration-150 flex flex-col max-h-[90vh]">
        ...
        <div className="p-lg pb-md border-b border-border-subtle bg-surface-container-low flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 id="manage-role-modal-title" className="font-headline-md text-headline-md text-on-surface font-semibold">
                Manage Role &amp; Status
              </h3>

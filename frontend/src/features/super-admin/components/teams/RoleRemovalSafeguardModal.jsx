        ...
        <form onSubmit={onSubmit} className="p-lg space-y-md">
          <div className="p-sm rounded-xl bg-surface-container-low border border-border-subtle space-y-1 text-body-sm">
            <p className="text-on-surface font-semibold">
              {data.member?.name} is losing their primary role &ldquo;{data.roleToRemove}&rdquo;.
            </p>
            <p className="text-on-surface-variant text-[12px] leading-relaxed">
              To prevent orphaned members and ensure uninterrupted workspace permissions in <strong>{data.team?.name}</strong>, please select a replacement role.
            </p>
          </div>
          ...
          <div className="space-y-xs">
            ...
            >
              {availableRoles
                .filter((role) => role.name !== data.roleToRemove)
                .map((role) => (
